...
class RoomCharacterClassStorage(private val database: CharacterCreationDb, private val concurrency: Concurrency) : DndCharacterClassStorage {
	...
	override fun storeSelection(selection: DndCharacterClassSelection, id: CharSequence?): CharSequence {
		...
		concurrency.runDiskOrNetwork({
			...
			database.classDao().storeClassSelection(StoredCharacterClassSelection(createdOrExistingId.toString()))
			...
			for (state in classesToStore) {
				...
				database.classDao().storeSelectionClassJoin(StoredSelectionCharacterClassJoin(dndClass.detailsUrl, createdOrExistingId.toString(), stateAsInt))
			}
		})
